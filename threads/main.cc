...
#ifdef THREADS
extern void problem2(void), TestSuite(void),SenateFunc(),SenateFuncDync();
#endif

#ifdef NETWORK
	extern void Server_RPC();
#endif
...
main(int argc, char **argv)
{
	...
	if (!strcmp(*argv, "-cp")) { 		// copy from UNIX to Nachos
		...
		{	
			visitornum=atoi(*(argv+1));
			senatornum=atoi(*(argv+2));
			operatornum=atoi(*(argv+3));
			Delay(2);
			Server_RPC();
		}
#endif // NETWORK
