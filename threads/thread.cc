...
IntStatus oldLevel = interrupt->SetLevel(IntOff);									//u
	...
	{	
		...
		if (machine->tlb[m].dirty == TRUE)
			{
				//for(int n=0;n<currentThread->space->numPages;n++)
					{	
						//if(machine->tlb[m].virtualPage == n)
						//	currentThread->space->pageTable[n].dirty = TRUE;
				...
				/*
				IPT machine->tlb[m].physicalPage can also be used.
				*/
			}
	}
	
	(void) interrupt->SetLevel(oldLevel);												//u
